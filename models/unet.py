...
def get_unet(unet, in_channels):
    ...
    n_enc_convs = n_conv
    n_dec_convs = n_conv
    print(f"[UNet]         enc_channels={enc_channels}")
    print(f"[UNet]         dec_channels={dec_channels}")
    print(f"[UNet]         n_enc_convs={n_enc_convs}")
    print(f"[UNet]         n_dec_convs={n_dec_convs}")
    if slen > 0:
        print(f"[UNet]         n_seq={n_seq}")
        print(f"[UNet]         refine_add={refine_add}")
        return SeqUNet(
            n_seq=n_seq,
            in_channels=in_channels,
            enc_channels=enc_channels,
